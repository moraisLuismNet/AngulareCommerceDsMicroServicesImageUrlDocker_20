...
export class GroupsComponent {
  ...
  onImageUrlChange(value: string) {
    ...
    console.log(
      'onImageUrlChange - group.imageGroup antes:',
      this.group.imageGroup
    );
    this.group.imageGroup = value;
    console.log(
      'onImageUrlChange - group.imageGroup después:',
      this.group.imageGroup
    );
    this.cdr.markForCheck();
  }
  ...
  save() {
    ...
    const imageValue = this.group.imageGroup
      ? this.group.imageGroup.trim()
      : null;
        ...
        .subscribe({
          next: (data) => {
            ...
            if (data && data.imageGroup === null && groupToSave.imageGroup) {
              console.log(
                'El servidor devolvió imageGroup como null, usando el valor original'
              );
  ...
  async edit(group: IGroup) {
    ...
    setTimeout(() => {
      ...
      const selectedGenre = this.genres.find(
        (g) => g.idMusicGenre === this.group.musicGenreId
      );
          ...
          Object.keys(err.error.errors).forEach((key: string) => {
            if (Array.isArray(err.error.errors[key])) {
              validationErrors.push(
                ...err.error.errors[key].map((e: any) => e.toString())
              );
            } else {
